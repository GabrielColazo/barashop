...
- **Header redesign (jul 2026):** Logo reducido a 60px desktop / 48px mobile (original 90px). Clase `.header-link` para links de texto (Contacto con envelope SVG, Ingresar). Avatar con SVG barquito (`.avatar-barco`) reemplaza la inicial del email en todos los headers y offcanvas. Contacto: envelope SVG inline + header-link. Ingresar: de `btn-secundario` a `header-link`. Eliminado `js/mobile-menu.js` (código muerto tras migración offcanvas).
- **Precio a convenir (jul 2026):** Nuevo chip `.toggle-a-convenir` en publicar.html — solo visible en categoría Servicios. Al tildar, deshabilita campo precio (como Donaciones), guarda `null` en DB, muestra "A convenir" en tarjetas (index, mis-avisos) y detalle (anuncio.html). Al editar aviso `null`, checkbox se carga tildado automáticamente. CSS: chip azul (#1D4ED8) con estado `.activo`. Validación submit: `aConvenir` bypass check de precio > 0.
- **Fix: JS duplicado en publicar.html (jul 2026):** Bloque `labelAConvenir`/`checkAConvenir` quedó duplicado fuera de `seleccionarCategoria()` — rompía todo el script al cargar (foto no subía, "a convenir" aparecía en todas las categorías). Eliminada la instancia duplicada, conservando la correcta dentro de `seleccionarCategoria()`.
- **Favicon actualizado (jul 2026):** Nuevos archivos en `assets/img/favicon/` (favicon.ico, favicon-32x32.png, favicon-16x16.png, apple-touch-icon.png). Tags `<link rel="icon">` agregados a index.html (la única página que no los tenía).
- **Logo agrandado + padding compensado (jul 2026):** Logo 72px desktop / 56px mobile (antes 60/48). Header padding reducido (0.15rem desktop / 0.35rem mobile) para que el alto total del header no crezca.
- **Empty state rediseñado (jul 2026):** index.html diferencia dos casos: sin avisos en el sitio ("Todavía no hay avisos" + botón Publicar) vs búsqueda/filtro sin resultados ("No encontramos resultados" + botón Limpiar filtros). Ilustración animada: barquito SVG con lupa orbitando en CSS puro (`@keyframes orbitLupa`, 3.5s lineal infinito). Cuando hay categoría activa sin resultados, el texto menciona la categoría ("Sé el primero en publicar en [categoría]"). mis-avisos.html no se tocó.
- **Fix: empty state categoría vacía (jul 2026):** Seleccionar una categoría sin avisos mostraba "No encontramos resultados" (era `!!categoriaActiva` en `hayFiltro`). Fix: solo `!!busqueda` dispara el mensaje de "sin resultados"; categoría sin búsqueda usa "Todavía no hay avisos".
- **Soporte SVG en íconos de categoría (jul 2026):** Nueva función `renderIcono()` en las 4 páginas con renderizado de categorías (index, publicar, mis-avisos, anuncio). Detecta si el campo `icono` empieza con `<svg` → lo devuelve tal cual; si es emoji → lo escapa con `escapeHtml()`. CSS: `.cat-icon svg { width:20px; height:20px; display:block }` en `_components.scss`. Permite usar SVG inline en el campo `icono` de la tabla `categorias` de Supabase sin romper emojis existentes.
- **Fix: XSS en barra de categorías (jul 2026):** Las líneas que insertaban `cat.icono` y `cat.nombre` en `cargarCategorias()` (index.html) usaban innerHTML sin escapar. Corregido: `renderIcono(cat.icono)` + `escapeHtml(cat.nombre)`.
- **schema.sql sincronizado con RLS (jul 2026):** Policies actualizadas: INSERT `anuncio_imagenes` ahora verifica que el usuario sea dueño del anuncio (`EXISTS SELECT 1 FROM anuncios WHERE...`). UPDATE `anuncios` agregado `WITH CHECK (auth.uid() = usuario_id)` para prevenir transferencia de propiedad.
...
- **Fix: avatar SVG width todas las páginas (ago 2026):** Selector `#user-avatar svg { width: 22px }` solo aplicaba a index.html. Corregido a `#user-avatar svg, [id$="-user-avatar"] svg { width: 22px; height: auto }` para cubrir todos los IDs: `pub-user-avatar` (publicar), `mis-user-avatar` (mis-avisos), `detalle-user-avatar` (anuncio). Sin este fix, el SVG del barco no tenía tamaño en esas páginas y el avatar quedaba invisible.
